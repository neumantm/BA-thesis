...
Of the 12 experts, who agreed to participate, 7 actually took part in the review and survey.
...
Furthermore, the experts have very different amounts of experience.
One is working as an intern during his masters' studies, others have been developers for a few years and some are senior software engineers, who have been working in the industry for tens of years.

In general most experts appreciate the concept.
They also see potential in the extension, but would like a few more features.
In the following paragraphs, first the general and technical feedback for the extension is summarized.
Then the results of the expert survey are presented.
...
One expert also requested the ability to specify the kind of a relation between to issues.
...
Moreover, two expert stated, that they would like to be able to sort the list of labels for each issue in the issue list, such that, for example, the work-in-progress label is always the first one displayed.
Finally, one expert thinks that the issue locations would rarely be used in practice as it is to much extra work and most developers dislike managing such additional data.
...
Another especially liked the icons, because they allow to quickly get an overview over the issues and some of their properties without being overloaded.
He also liked, that there are not to many types of issues.
...
The next expert stated, that the most significant problem is tracking issues across multiple projects, that depend on each other.
...
He also stated that he often misses the ability to specify the kind of a relation between two issues or the different types of relations are to restricted.
On the other hand, he thinks that most \glspl{IMS} have to many different types of issues, which causes confusion and allows to much room for interpretation.
...
One expert also sees the problem, that developers need to keep the overview over the components, to know in which \gls{IMS} they need to create or search an issue.
...
The expert, who doesn't see any problem with existing \gls{IMS} when working with issues affecting multiple independent projects, states that the problem is seeing this link to a line in his \gls{IDE}.
...
Two expert agrees, that this is a problem with existing \glspl{IMS}.
...
Two experts also stated, a graphical overview over the linked issues would be nice.
...
According to another expert the concept reduces these problems as well as the need of keeping an overview over the components and their respective \gls{IMS} but the implementation still needs some work.
...
Another expert also states, that navigating between issues is easy with \gls{GropiusEI}, but the links should have more than one type for even better overview.
...
As all major \glspl{IMS} are web-based, whenever they link to the source code, it is also opened in the browser,
where various \gls{IDE} features for exploring or editing the code are not available.
